...
start:
    ...
    then 
        ...
        curl --insecure "https://${DEV_SITE}/" --data-raw 'forumTitle='${SITE_TITLE}'&mysqlHost=db&mysqlDatabase='${MYSQL_DATABASE}'&mysqlUsername='${MYSQL_USER}'&mysqlPassword='${MYSQL_PASSWORD}'&tablePrefix=&adminUsername='${SITE_ADMIN}'&adminEmail=null%40null.null&adminPassword='${SITE_ADMIN_PASS}'&adminPasswordConfirmation='${SITE_ADMIN_PASS}
    fi
stop:
...
recreate-database:
    ...
    curl "https://${DEV_SITE}"/api/discussions --insecure -H "Authorization: Token ${MASTER_TOKEN}; userId=1" -H 'Content-Type: application/json; charset=utf-8' --data-raw '{"data":{"type":"discussions","attributes":{"title":"INTRO","content":"Hello"},"relationships":{"tags":{"data":[{"type":"tags","id":"1"}]}}}}'
setup:
    #!/usr/bin/env bash
